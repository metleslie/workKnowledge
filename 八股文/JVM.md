...
说说为什么有垃圾回收机制，在我们之前写C或C++的时候有个点一直被人诟病，就是在你开辟空间以后，需要及时回收，不然的话就会无休止的创建空间，内存就能爆掉，但人不是机器，不可能面面俱到，就有可能在某些小的地方创建完对象后忘记回收，后面可能会导致内存用量一点点增加，导致爆掉。

那我们既然有垃圾回收机制，就要有垃圾判断算法，不然怎么回收
### 垃圾判断算法
两种垃圾算法：
- 可达性分析算法
- 引用计数算法
...
#### 引用计数算法
通过对象头里面分配一块空间用来保存，当前对象被引用了多少次。
如果该对象被其他对象引用了，次数+1，如果删除该对象的引用，那么它的引用计数就会-1，当该对象的引用次数为0的时候，就会被垃圾回收。

引用计数算法将垃圾回收分摊到整个应用程序的运行当中，而不是在垃圾集中回收时，和我们的垃圾收集的STW不一样。

缺点：循环依赖解决不了，A引用B，B引用A，那么他们的计数永远不为0，导致永远不会被回收。

#### 可达性分析算法
基本思路为，通过GC roots作为起点，然后向下搜索，搜索走过的路径被称为rereference chain（引用链），当一个对象达到GC roots的时候没有任何引用相连，即GC roots到达该对象没有引用，即表示不可达，被称为垃圾。

通过可达性分析算法，成功解决了循环依赖的问题，只要你没有直接或间接的连接与GC roots,系统就会判定你为可回收对象。

GC roots是一组数据，并且是动态的，只要里面的一个数据直接或间接到达一个对象，就证明这个对立面的对象是可达的活跃的不能被回收。

- **静态变量：** 属于类，生命周期与类相同（通常伴随 ClassLoader）。
    
- **活动线程的栈帧中的局部变量：** 方法正在执行，其局部变量引用的对象当然不能回收。
    
- **常量池引用：** 字符串常量等需要长期存活。
    
- **JNI 引用：** 本地代码（C/C++）可能还在使用这些对象。
    
- **被同步锁持有的对象：** 一个线程持有锁的对象，很可能还在被该线程使用。
    
- **JVM 内部引用：** 类对象、系统类加载器、一些 JVM 内部数据结构等。
这些作为GC roots是因为这些来源的引用，都指向着当前绝对不能被回收的对象。把他们作为根才能确保垃圾回收不会错误的回收正在使用的对象。

### STW
它是垃圾回收机制的一个概念，在JVM中，垃圾回收会停止所有的线程活动，因为有可能造成引用地址改变，所以需要重新加载地址，这个过程，不能活动线程。
...
因此，在==选择和调优垃圾收集器时，需要考虑其停顿时间==。Java 中的一些垃圾收集器，如 G1 和 ZGC，都会尽可能地减少了"Stop The World"的时间，通过并发的垃圾收集，提高应用的响应性能。

总的来说，"Stop The World"是 Java 垃圾收集中必须面对的一个挑战，其目标是在==保证内存的有效利用和应用的响应性能之间找到一个平衡。==

### 垃圾收集算法
在确定了哪些是垃圾后，GC要做的是收回垃圾，但是这里面涉及到一个问题：如何高效的进行垃圾回收。不同厂商的JVM有着不同的垃圾回收算法。

#### 标记清除算法
最基础的垃圾算法，分为两部分，先把内存区域里的对象进行标记，哪些属于可回收的标记出来，然后进行清理。非常简单且清晰。
缺点：内存碎片太多，当对象需要比较大的空间时可能没有足够空间导致重新进行垃圾收集。

### 复制算法
标记清除算法演化而来，解决碎片问题，它由两块同样大小的内存组成，当一块内存使用完毕，就进行垃圾清除，然后把剩余的放到另一块新的上面。
缺点：空间利用率过低。

### 标记整理
标记过程仍然和清除算法一致，但后面会让存活的对象向一段移动，清理掉边界以外的内存。
标记整理算法一方面在标记-清除算法上做了升级，解决了内存碎片的问题，也规避了复制算法只能利用一半内存区域的弊端。

缺点：内存变动频繁，需要整理所有存活对象的引用地址。

### 分代收集算法
就是我们说的新生代老年代这种算法，它融合了上面的所有算法，分为新生代和老年代。
在新生代中，每次垃圾收集时都发现有大批对象死去，只有少量存活，那就选用复制算法，只需要付出少量存活对象的复制成本就可以完成收集。

老年代中因为对象存活率高、没有额外空间对它进行分配担保，就必须使用标记清理或者标记整理算法来进行回收。

## 新生代和老年代
堆是JVM里面最大的一块内存，也是垃圾管理器主要作用的区域。
堆主要分为两个区，新生代和老年代。新生代分为EDEN区和两个s区

#### Eden区
研究表明，百分之98的对象都是存活时间极短，针对这一现象，大部分对象会在Eden区进行GC，活着的对象会进入到S0fron区，如果s0空间不够，则进入s1

#### S区
相当于一个缓冲区

为什么需要S区，没有S区的话，那么老年区将很快被填满，但是大部分对象其实也就是两三轮的GC回收就清除了，S区就是要减少对象进入老年区，减少MajorGC的发生，它的与筛选保证经过16次minorGC才进入老年区

为什么两块，我们在进行minorgc，Eden区包括s区都有可能有要清理的对象，活着的进入s0，进行下一次时，两块s区互换，这样我们就会有一片连续的空间，不用大片的内存碎片产生。

### Old区
老年区占总共的三分之二，只有在MajorGC的时候才会清理，每次清理都触发STW机制，STW的时间越长，对项目越不利，所以内存不仅仅是越大越好。
复制算法会进行多次复制操作效率低，老年代采用标记标记整理算法。
除了上述的内存担保机制，无法安置的对象会进入老年代，还有几种情况会进入老年代。

- 大对象：过大的对象，Eden区装不下
- 长期存活：经过s区15次GC
- 动态对象年龄：==如果s区某个年龄或以上的对象总大小超过了S区的一半，那么该年龄以上的对象会在下一次垃圾回收进入到老年代。这种动态调整机制有助于优化内存使用和减少垃圾收集的频率，特别是在处理大量短生命周期对象的应用程序时。==

## 2.Minor GC，MajorGC，Mixed GC，FullGC分别是什么

### MinorGC
YoungGC是发生在新生代的垃圾回收，Eden区和两个S区，发生GC的时候Eden区和s0区进行回收，活着的到另一个S区，下一次两个S区反过来
### MajorGC
老年代的垃圾回收，独属于CMS。

### MixedGC
G1垃圾收集器特有，一次GC同时清理年轻代和部分老年代。
...
## 3.minorGc Fullgc触发条件
Eden区没有足够空间就会触发Minorgc清理新生代

进行youngGC时，如果发现老年代可用的连续空间<新生代历次MinorGC后升入老年代总和的平均，说明本次MinorGC后升入老年代的大小超过老年代可用的空间就会触发FullGC

执行YoungGC后老年代没有足够的内存空间存放存活对象，会立即进行FullGC

空间分配担保
MinorGC后存活的对象一定要小于老年代剩余空间。

## 4.知道哪些垃圾收集器？
主要分为分代收集齐和分区收集器

### 分代收集器
#### **分代垃圾收集器 (Generational GC)**

- **核心假设**：基于 **弱分代假说**（Weak Generational Hypothesis）：
    
    - 绝大多数对象都是“朝生夕死”的（年轻代很快消亡）。
        
    - 存活时间越长的对象越难消亡（老年代长期存活）。
        
- **内存划分**：
    
    - 将堆内存分为 **年轻代（Young Generation）** 和 **老年代（Old Generation）**：
        
        - **年轻代**：进一步分为 **Eden区** 和 **Survivor区**（S0/S1）。
            
        - **老年代**：存放长期存活的对象。
            
    - **元空间（Metaspace）**：独立于堆，存储类元数据。
        
- **典型代表**：  
    `Serial`、`Parallel Scavenge`、`ParNew`、`CMS`（老年代收集器）。

### 分区收集器
- **核心思想**：将堆内存划分为 **多个大小相等的独立区域（Region）**，每个区域可独立回收。
    
- **内存划分**：
    
    - 堆被拆分为数百至数千个 **固定大小的 Region**（如 1MB~32MB）。
        
    - 每个 Region 可动态分配给不同分代（如 G1），或完全不分代（如 ZGC/Shenandoah）。
        
- **目标**：  
    实现更可控的停顿时间（低延迟），避免全堆扫描。
    
- **典型代表**：  
    `G1`（分代+分区）、`ZGC`、`Shenandoah`（纯分区，不分代）。
...
### G1收集器
现在JDK8默认用的一般是G1收集器，它是逻辑分代，物理分区的代表
1. **堆被划分为固定大小的 Region**  
    G1 将整个堆（包括年轻代和老年代）划分为 **多个大小相等的 Region**（默认约 2048 个，每个 Region 大小在 1MB~32MB 之间可调）。
    
    - **所有 Region 物理上等价**：不再有传统的 Eden、Survivor、Old 物理隔离。
        
2. **动态分配 Region 角色**  
    每个 Region 在运行时被**动态分配**为以下逻辑角色之一：
|**Eden Region**|存放新分配的对象（年轻代）|
|**Survivor Region**|存放 Minor GC 后存活的对象（年轻代）|
|**Old Region**|存放长期存活的对象（老年代）|
|**Humongous Region**|存放大对象（大小 > Region 50%）|
|**Free Region**|未分配的空闲区域|

尽管内存是分区管理的，G1 **仍然保留分代收集的逻辑**：

1. **对象年龄跟踪（Age Tracking）**
    
    - 对象每次在 Young GC 中存活，年龄 `+1`。
        
    - 当年龄 **超过阈值**（默认 `15`）时，从年轻代晋升到老年代。
        
2. **分代回收策略**
    
    - **年轻代回收（Young GC）**：  
        只回收所有 **Eden Regions + Survivor Regions**（复制算法）。
        
    - **混合回收（Mixed GC）**：  
        在回收年轻代的同时，**选择性回收部分老年代 Regions**（优先选垃圾最多的 Region）。


    **现代 JVM 的选择**：

- JDK 8 默认 `Parallel Scavenge`（分代），但建议手动切 `G1`（分区）。
    
- **JDK 11+ 默认 `G1`**，超大堆或超低延迟场景可用 `ZGC`/`Shenandoah`（纯分区）。
    
- 分代是逻辑概念（对象年龄），分区是物理概念（内存块）。**G1 是分代与分区的融合体**，而 ZGC/Shenandoah 彻底抛弃分代。